...
bakers <- read_csv("data/bakers.csv") %>%
  janitor::clean_names()
  # Rename the 'baker_name' column to 'baker'
bakers <- bakers %>%
...
results <- read_csv("data/results.csv") %>%
  janitor::clean_names()
# Re-read the results dataset, skipping the metadata rows
...
```{r}
# Check bakers in bakes but not in bakers
missing_bakers_in_bakes <- bakes %>%
  select(baker) %>%
  distinct() %>%
  anti_join(bakers, by = "baker")

# Check bakers in results but not in bakers
missing_bakers_in_results <- results %>%
  select(baker) %>%
  distinct() %>%
  anti_join(bakers, by = "baker")
...
missing_bakers_in_results

```


```{r}
# Merge bakes and bakers datasets
combined_data <- left_join(bakes, bakers, by = "baker")
